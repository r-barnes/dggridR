---
title:  "dggridR: Spatial Statistics Done Right"
author:
- Richard Barnes
- Kevin Sahr
date: "`r Sys.Date()`"
...
vignette: >
  %\VignetteIndexEntry{dggridR: Spatial Statistics Done Right}
...
many of these grids will also incorporate a small number of pentagons, as
detailed below. However, the default orientation of the grids is such that these
are located over the most inaccessible backwaters of the Earth's oceans. Future
versions of this package will better explain this limitation. (TODO))

##Notes

Many different grid resolutions are available for many different grids. Grid
types, and some details about them follow.

For each of the following three grids, at every resolution there are 12 pentagonal
cells. Every other cell is a hexagon. The pentagons have an area exactly 5/6 the
area of a hexagon.
...
In the above grids cells from one resolution level are partially contained by
the cells of other levels:
...
User Documentation, which is available in its entirety [here](#TODO).
