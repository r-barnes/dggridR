...
===========================================================================
   ...
   Kevin Sahr: modified for single-time creation of sb_tree, 8/1/01
   Kevin Sahr: modified to use long double 1/20/11
   Kevin Sahr: ported to C++
*/
...
#define EQ(a, b)           (fabsl((a) - (b)) <= GPC_EPSILON)
...
#define MALLOC_ORIG(p, b, s)    {if ((b) > 0) { \
                            p= malloc(b); if (!(p)) { \
...
{
  long double              x;            /* X coordinate component            */
  long double              y;            /* Y coordinate component            */
...
typedef struct edge_shape
{
  ...
  long double              xb;           /* Scanbeam bottom x coordinate      */
  long double              xt;           /* Scanbeam top x coordinate         */
  long double              dx;           /* Change in x for a unit y increase */
...
{
  long double              y;            /* Y coordinate at local minimum     */
...
static edge_node **bound_list(lmt_node **lmt, long double y)
{
...
/* altered by K. Sahr */
static void add_to_sbtree(int *entries, sb_tree **sbtree, long double y, 
                          int firstPoly)
{
  if (!*sbtree)
  {
    ...
    (*sbtree)->less= NULL;
    (*sbtree)->more= NULL;
    (*sbtree)->parPtr = sbtree;
    (*sbtree)->firstPoly = firstPoly;
    (*entries)++;
  }
  else
  {
    ...
    /* Head into the 'less' sub-tree */
      add_to_sbtree(entries, &((*sbtree)->less), y, firstPoly);
    }
    else
    {
      if ((*sbtree)->y < y)
      {
        ...
        add_to_sbtree(entries, &((*sbtree)->more), y, firstPoly);
      }
    }
  }
}
...
/*
JFW: These do not appear to ever be used; until source control
is better-established, I'll stick with the time-honored "comment-it-out"
tradition:
static void print_sbtree_helper(sb_tree *sbtree)
{
   if (!sbtree)
      printf("NULL\n");
   else
   {
      printf(" {\n");
      printf(" y: %Lf  LESS => ", sbtree->y);
      print_sbtree_helper(sbtree->less);
      printf(" y: %Lf  MORE => ", sbtree->y);
      print_sbtree_helper(sbtree->more);
      printf(" }\n");
   }
}

static void print_sbtree(int entries, sb_tree *sbtree)
{
   printf("SBTREE:: #entries: %d\n", entries);
   printf("{\n");
   print_sbtree_helper(sbtree);
   printf("}\n");
}
*/
...
{
  int c, i;
  int *entries = &(p->sbtentries);
  sb_tree **sbtree = &(p->sbtree);
    ...
    if (p->contour[c].num_vertices >= 0)
    {
      for (i= 0; i < p->contour[c].num_vertices; i++)
        if (OPTIMAL(p->contour[c].vertex, i, p->contour[c].num_vertices))
        {
          ...
          add_to_sbtree(entries, sbtree, p->contour[c].vertex[i].y, 1);
        }
     }
   }

   p->box = create_contour_bboxes(p);
}

static void build_sbt(int *entries, long double *sbt, sb_tree *sbtree)
{
  if (sbtree->less)
...
/* added by K. Sahr */
static void free_sbtree_count(sb_tree **sbtree, int *entries)
{
  if (*sbtree)
  {
    free_sbtree_count(&((*sbtree)->less), entries);
    free_sbtree_count(&((*sbtree)->more), entries);
    FREE(*sbtree);
    (*entries)--;
  }
}

static void free_sbtree_partial(sb_tree **sbtree, int *entries)
{
  if (*sbtree)
  {
    if ((*sbtree)->firstPoly)
    {
       free_sbtree_partial(&((*sbtree)->less), entries);
       free_sbtree_partial(&((*sbtree)->more), entries);
    }
    else
    {
       free_sbtree_count(sbtree, entries);
    }
  }
}
/* end of addition */
...
void print_edge_node (edge_node *e)
{
   printf("(%.4Lf, %.4Lf) [(%.4Lf, %.4Lf)->(%.4Lf, %.4Lf)], %.4Lf %.4Lf "
          "%.4Lf %d %d %d %d %d %d %d %d %d\n",
       e->vertex.y, e->vertex.x,
       e->bot.x, e->bot.y, e->top.x, e->top.y,
       e->xb, e->xt, e->dx, e->type,
       e->bstate[ABOVE], e->bstate[BELOW],
       e->bundle[ABOVE][CLIP], e->bundle[ABOVE][SUBJ],
       e->bundle[BELOW][CLIP], e->bundle[BELOW][SUBJ],
       e->bside[CLIP], e->bside[SUBJ]);
}

/* altered by K. Sahr */
static edge_node *build_lmt(lmt_node **lmt, sb_tree **sbtree,
                            int *sbt_entries, gpc_polygon *p, int type,
                            gpc_op op, int firstPoly)
{
...
/* this is so that we can compare */
    for (i = 0; i < total_vertices; i++)
    {
       ...
       e->dx= 0.0L;
       e->type= 0;
       e->bstate[ABOVE]= UNBUNDLED;
       e->bstate[BELOW]= UNBUNDLED;
       e->bundle[ABOVE][CLIP]= FALSE;
       e->bundle[ABOVE][SUBJ]= FALSE;
       e->bundle[BELOW][CLIP]= FALSE;
       e->bundle[BELOW][SUBJ]= FALSE;
       e->bside[CLIP]= 0;
       e->bside[SUBJ]= 0;
       e->outp[ABOVE]= NULL;
       e->outp[BELOW]= NULL;
        ...
        if (OPTIMAL(p->contour[c].vertex, i, p->contour[c].num_vertices))
        {
          ...
          if (!firstPoly) 
            add_to_sbtree(sbt_entries, sbtree,
                          edge_table[num_vertices].vertex.y, 0);
...
static void build_intersection_table(it_node **it, edge_node *aet, long double dy)
{
  st_node   *st, *stp;
  edge_node *edge;
...
static void add_left(polygon_node *p, long double x, long double y)
{
  vertex_node *nv;
...
/*
  printf("merge_left: %d %d\n", list->active, list->hole);
*/
}


static void add_right(polygon_node *p, long double x, long double y)
{
  vertex_node *nv;
...
static void add_vertex(vertex_node **t, long double x, long double y)
{
  if (!(*t))
  {
...
void gpc_read_polygon(FILE *fp, int read_hole_flags, gpc_polygon *p)
{
  ...
  for (c= 0; c < p->num_contours; c++)
  {
    if (!fscanf(fp, "%d", &(p->contour[c].num_vertices)))
       exit(1);

    if (read_hole_flags)
    {
      if (!fscanf(fp, "%d", &(p->hole[c])))
         exit(1);
    }
    else
                      ...
                      gpc_polygon *result)
{
  ...
  long double        *sbt= NULL, xb = 0.0L, px = 0.0L, yb = 0.0L, yt = 0.0L, dy = 0.0L, ix = 0.0L, iy = 0.0L;
  unsigned      char firstPolySBT = 0;
  ...
  {
    if (firstPolySBT) /* don't need to create the sbt for this poly */
    {
      sbtree = subj->sbtree;
      sbt_entries = subj->sbtentries;

      s_heap= build_lmt(&lmt, &sbtree, &sbt_entries, subj, SUBJ, op, 1);
    }
    else
    {
      s_heap= build_lmt(&lmt, &sbtree, &sbt_entries, subj, SUBJ, op, 0);
    }
  }

  if (clip->num_contours > 0)
    c_heap= build_lmt(&lmt, &sbtree, &sbt_entries, clip, CLIP, op, 0);
  ...
  MALLOC(sbt, sbt_entries * sizeof(long double), "sbt creation", long double);
  build_sbt(&scanbeam, sbt, sbtree);
  scanbeam= 0;
  if (firstPolySBT)
  {
    nsbt = sbt_entries;
    free_sbtree_partial(&sbtree, &nsbt);
    if (nsbt != subj->sbtentries)
    {
      fprintf(stderr, "SBTENTRIES %d vs. %d\n", nsbt, subj->sbtentries);
    }
  }
  else
  {
...
/*
printf("vclass %d  contrig %d  tr %d  tl %d  br %d   bl %d\n",
         vclass, contributing, tr, tl, br, bl);
*/
...
/* created by K. Sahr */
int gpc_polygon_intersect(gpc_polygon *subj, gpc_polygon *clip)
{
  gpc_op        op = GPC_INT;
  sb_tree       *sbtree= NULL;
  it_node       *it= NULL, *intersect;
  ...
  edge_node     *aet= NULL, *c_heap= NULL, *s_heap= NULL;
  lmt_node      *lmt= NULL, *local_min;
  polygon_node  *out_poly= NULL, *p, *q, *cf= NULL;
  h_state        horiz[2];
  int            in[2], exists[2], parity[2]= {LEFT, LEFT};
  int            contributing, search, scanbeam= 0, sbt_entries= 0;
  int            nsbt = 0;
  ...
  /* if we're here we don't have an intersection */

  if (count_contours(out_poly) > 0)
  {
    fprintf(stderr, "FATAL ERROR: out_poly in gpc_polygon_clip2\n");
    exit(-1);
  }
                       ...
                       gpc_tristrip *result)
{
  ...
  int            vclass = 0, bl = 0, br = 0, tl = 0, tr = 0;
  long double        *sbt= NULL, xb = 0.0L, px = 0.0L, nx = 0.0L, yb = 0.0L, yt = 0.0L, dy = 0.0L, ix = 0.0L, iy = 0.0L;
