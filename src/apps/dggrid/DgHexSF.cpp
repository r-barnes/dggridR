...
*/
   ...
   else // quadNum 1-10
   {
      ...
      if (ed && res_ == 1)
      {
         ...
         std::uint64_t i = ijkCoord().i();
         std::uint64_t j = ijkCoord().j();
         ...
         if (!useTile && cr.isQuadUsed())
         {
            ...
            std::int64_t midI = (dgg.maxI() + 1) / 2;
            std::int64_t midJ = (dgg.maxJ() + 1) / 2;
            std::int64_t left = 0, right = 0, top = 0, bottom = 0;
